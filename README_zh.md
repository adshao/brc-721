...
| uid | 否 | 签名使用的 `uid` 字段是否唯一，则后续所有铸造铭文的 `uid` 字段必须唯一，如果重复 `uid` 的铭文则无效，默认值 `false` |
| expt | 否 | 签名使用的 `expt` 字段是否有效，则后续所有铸造铭文的 `expt` 字段必须大于铭文所在区块的时间 `block_time`，否则铭文无效，默认值 `false` |
| exph | 否 | 签名使用的 `exph` 字段是否有效，则后续所有铸造铭文的 `exph` 字段必须大于铭文所在区块的高度 `block_height`，否则铭文无效，默认值 `false` |
...
| s | 否 | DER编码的签名，十六进制表示 |
| rec | 否 | 签名使用的 `rec` 字段，用于指定当前铭文的 `rec`，如果 `rec` 不是铭文的接收者，则铭文无效 |
| uid | 否 | 签名使用的 `uid` 字段，用于唯一指定当前铭文的 `uid`，如果 `uid` 重复，则铭文ID（token ID）最小的铭文生效，其他重复 `uid` 的铭文无效 |
| expt | 否 | 签名使用的 `expt` 字段，用于指定当前铭文的 `expt`，如果 `expt` 小于铭文所在区块的时间 `block_time`，则铭文无效 |
| exph | 否 | 签名使用的 `exph` 字段，用于指定当前铭文的 `exph`，如果 `exph` 小于铭文所在区块的高度 `block_height`，则铭文无效 |
...
* 原始签名信息 Message：将 `fields` 中指定的字段按照字典序排列，然后拼接成字符串，如：`exph=100&rec=xxx&uid=xxx`
