...
* 指定后续铸造铭文的签名必须使用 `sig.pk` 指定的私钥进行签名，否则铭文无效
...
| uid | 否 | 签名使用的 `uid` 字段是否唯一，如果提供，则后续所有铸造铭文的 `uid` 字段必须唯一，重复 `uid` 的铭文无效 |
| expt | 否 | 签名使用的 `expt` 字段是否有效，如果提供，则后续所有铸造铭文的 `expt` 字段必须大于铭文所在区块的时间 `block_time`，否则铭文无效 |
| exph | 否 | 签名使用的 `exph` 字段是否有效，如果提供，则后续所有铸造铭文的 `exph` 字段必须大于铭文所在区块的高度 `block_height`，否则铭文无效 |
